...
class SlowLorisAnalyzer(HackingTool):
    TITLE = "Slowloris Vulnerability Analyzer"
    DESCRIPTION = (
        "Analyzes a web server for vulnerability to Slowloris-style HTTP slow request attacks. "
        "Performs 8 non-destructive checks: header timeout, connection limits, rate limiting, "
        "WAF detection, server architecture, reverse proxy, CDN detection, and keep-alive config."
    )
    RUN_COMMANDS = ["python3 scripts/slowloris_analyzer.py --help"]

    def run(self):
        console.print(Panel(Text(self.TITLE, justify="center"), style=f"bold {P_COLOR}"))
        target = Prompt.ask("Enter Target (hostname or URL)")
        port = Prompt.ask("Enter Port", default="80")
        subprocess.run(["python3", "scripts/slowloris_analyzer.py", "--target", target, "--port", port])


class DDOSTools(HackingToolsCollection):
    TITLE = "DDOS Attack Tools"
    TOOLS = [SlowLoris(), Asyncrone(), UFONet(), GoldenEye(), Saphyra(), SlowLorisAnalyzer()]
